...
import { getAppsIndexPath, getBlueprintStoreRoot } from '../../lib/PathResolver.js'
import { loadServiceDefinition } from '../ServiceRegistry.js'

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const legacyStoreRoot = path.resolve(__dirname, '../../db/store')
const serverRoot = path.resolve(__dirname, '../../..')

const getStoreRoot = (): string => {
  const storeRoot = getBlueprintStoreRoot()
  const legacyBlueprintPath = path.join(legacyStoreRoot, 'system-blueprint.json')
  const blueprintPath = path.join(storeRoot, 'system-blueprint.json')

  if (!fs.existsSync(blueprintPath) && fs.existsSync(legacyBlueprintPath)) {
    fs.mkdirSync(storeRoot, { recursive: true })
    fs.copyFileSync(legacyBlueprintPath, blueprintPath)
  }

  return storeRoot
}

const getSystemBlueprintPath = (): string => path.join(getStoreRoot(), 'system-blueprint.json')

const getLocalHomeRoot = (): string => path.join(getStoreRoot(), 'homes')
...
export const getTenantHomeRoot = (): string => {
  ...
  return process.env.TENANT_HOME_ROOT ?? getLocalHomeRoot()
}

export const ensureBlueprintFile = (): void => {
  const systemBlueprintPath = getSystemBlueprintPath()
...
export const readBlueprint = (): Record<string, unknown> => {
  ensureBlueprintFile()
  const raw = fs.readFileSync(getSystemBlueprintPath(), 'utf8')
  return JSON.parse(raw) as Record<string, unknown>
}

export const writeBlueprint = (payload: Record<string, unknown>): void => {
  ensureBlueprintFile()
  fs.writeFileSync(getSystemBlueprintPath(), JSON.stringify(payload, null, 2), 'utf8')
}
